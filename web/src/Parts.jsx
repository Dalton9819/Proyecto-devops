    ...
    return (
        <div className={"part-group"}>
            ...
            {props.parts.map((partName) => {
                ...
                if (
                    exclusion &&
                    props.choices[exclusion.part] === props.spec.values[props.spec.parts[exclusion.part]][exclusion.key]
                ) {
                    return null;
                }
...
function PartPicker(props) {
    ...
    return (
        <div className={"item-picker picker"}>
            <label htmlFor={`part-${props.name}`} className={"part-title"}>
            ...
            <select id={`part-${props.name}`} onChange={onChange} value={props.current} className={"select-css"}>
                {Object.keys(props.values).map((itemName) => (
                    <option key={`${props.name}=${itemName}`} value={props.values[itemName]}>
            ...
            <fieldset id={`part-${props.name}`} value={props.current} onChange={onChange}>
                ...
                {Object.keys(props.values).map((colorName) => (
                    <label
                        ...
                        style={{ backgroundColor: props.values[colorName] }}
